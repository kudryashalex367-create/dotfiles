...
* 🪟 **WM/DE:** Niri 26.04 (Main Ribbon) / DriftWM (2D Canvas) / Hyprland / GNOME 50
* ⌨️ **Terminal:** Kitty (GPU-ускорение, JetBrains Mono Nerd Font, WGPU-блюр)
* 🐚 **Shell:** Zsh 5.9.2 + Oh My Zsh + Powerlevel10k (Lean 8-colors)
* 🎨 **UI Suite:** Fuzzel, Mako (Frosted Glass Notifications, 23MB RAM), Hyprlock, awww (144Hz Wipe Engine)
...
* 🖤 **Безбаровый Дзен (Barless Setup):** 100% площади экрана отдано контенту и коду — никаких лишних панелей задач и системного мусора.
* 🌊 **Кинематографичные пружины (Cinematic Silk 144Hz):** Физика пружин с критическим затуханием (ζ = 1.0) и замедлением 1.2 — шелковый ход ленты на 144 Гц без микро-фризов.
* 🖱️ **Умный фокус (Strict Hover Focus):** Фокус следует за мышью только для окон, на 100% находящихся в поле зрения (`max-scroll-amount="0%"`).
* 🧊 **Аппаратный Dual-Kawase Блюр:** Глубокое матовое стекло с дизерингом и сатурацией 1.20 для читаемости текста в Kitty и меню.
* 🔔 **Ультра-легкий Mako (23 MB RAM):** Замена тяжелых центров управления на легковесный Wayland-демон уведомлений в стиле Obsidian Matte.
* 🌌 **2D Spatial Canvas (DriftWM):** Экспериментальная сессия с бесконечным 2D-пространством, зумом камеры и магнитным прилипанием окон (Window Snapping).
* 🎲 **Рандомайзер обоев (`Win + A`):** Мгновенная плавная смена артов через `awww` на 144 Гц без открытия лишних меню.
* 📱 **Мобильный мост Termux:** Прямая связь со смартфоном по ADB/SSH с поддержкой 8-поточного скачивания через `aria2c`.
...
| Комбинация | Действие |
| :--- | :--- |
| `Super + Enter` | Открыть эмулятор терминала (Kitty) |
| `Super + D` | Меню запуска приложений (Fuzzel) |
| `Super + A` | **Случайные обои в 1 клик с 144Hz wipe-шторкой (`wall-random`)** |
| `Super + W` | Интерактивное меню выбора обоев из `~/wallpapers` (Fuzzel + `awww`) |
| `Super + V` | Менеджер истории буфера обмена (Cliphist + Fuzzel) |
| `Super + N` | **Восстановить последнее скрытое уведомление (`makoctl restore`)** |
| `Super + Shift + N` | **Очистить и закрыть все активные уведомления (`makoctl dismiss -a`)** |
| `Super + L` | Заблокировать экран (Hyprlock со сбросом раскладки на US) |
| `Super + O` | Панорамный обзор всех рабочих столов (Overview) |
| `Super + /` | Всплывающая интерактивная шпаргалка по хоткеям Niri |
...
| `Super + 1..9` | Прямой переход на рабочий стол 1..9 |
| `Super + Shift + 1..9` | Перенести колонку на рабочий стол 1..9 |
